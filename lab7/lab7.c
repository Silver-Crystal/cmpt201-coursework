...
void groupByKeyWeird(IntermediateInput *input, Output *output, int *result_count);
...
void groupByKeyWeird(IntermediateInput *input, Output *output, int *result_count) {
  ...
  if (output[input->doubled_value].count == 0) {
    output[input->doubled_value].doubled_value = input->doubled_value;
    (*result_count)++;
  }
  output[input->doubled_value].line_numbers[output[input->doubled_value].count] =
      input->line_number;
  output[input->doubled_value].count++;
}
...
void groupByKey(IntermediateInput *input, Output *output, int *result_count) {
  ...
  if (flag) {
    ...
    in = *result_count;
    output[in].doubled_value = input->doubled_value;
    (*result_count)++;
  }
  output[in].line_numbers[output[in].count] = input->line_number;
  output[in].count++;
}
void reduce(Output *output) {
  // TODO:
