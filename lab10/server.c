 ...
 *      the client uses sockets over the 'internet'(AF_INET instead of UNIX)
 *        the client uses TCP (NOT UDP) which can be seen by the protocol specifier 'SOCK_STREAM"
 *        (instead of dgram or smth)
 *  2. What data is the client sending to the server?
 *      The client is sending a fixed list of words one by one (i.e the words Hello, Apple, Car and
 *      so on(the count being 'NUM_MSG' messages)
 * Understanding server:
 *  1. Explain the argument that the 'run_acceptor' thread is passed as an argument.
 *    According to syntax for creating threads(that we can check using man pthread_create),
 *    it takes 4 arguments, the last seond of which is the function that we want to run, and the 4th
 *    argument being the arguments for that function.
 *    As run_acceptor is the thread that we need to continue accepting new connections and spawn new
 *    threads, that is passed as an argument
 *    it also needs to restrict the number of connections.
 *  2. How are received messages stored?
 *    Received messsages are stored in somewhat of a linked list(using list_node *'s)
 *  3. What does `main()` do with the received messages ?
 *    main 'collects' the messages(prints the messages  in the format ```Collected: $MESSAGE ```)
 *  4. How are threads used in this sample code?
 *    There are two ways, one, first a thread that accepts connections is created, followed by
 *    threads that act on those connections(run_client), these threads run while a few conditions
 *    are met(possibly till we have enough messages or wtv)
 */
#include <arpa/inet.h>
#include <errno.h>
#include <fcntl.h>
...
static void *run_acceptor(void *args) {
  ...
  for (int i = 0; i < num_clients; i++) {
    ...
    // pthreaed join
    // for things like stack, and the timer and registers
